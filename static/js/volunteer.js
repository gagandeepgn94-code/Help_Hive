...
function updateClock() {
  const el = document.querySelector('#liveTime span');
  if (el) {
    const now = new Date();
...
// ===== Leaflet Map =====
function initMap() {
  if (!volunteerLat && !volunteerLon) return;

  var map = L.map('map').setView([volunteerLat, volunteerLon], 14);
  ...
  L.marker([volunteerLat, volunteerLon], { icon: volunteerIcon })
    .addTo(map)
  ...
  requestData.forEach(req => {
    ...
    L.marker([req.lat, req.lon], { icon: requestIcon })
      .addTo(map)
      .bindPopup(
